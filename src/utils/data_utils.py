...
import os
from src.utils import forward_kinematics
...
def expmap2rotmat_torch(r):
    ...
    """
    r = r.cuda()
    theta = torch.norm(r, 2, 1).cuda()
    r0 = torch.div(r, theta.unsqueeze(1).repeat(1, 3) + 0.0000001)
    r1 = torch.zeros_like(r0).repeat(1, 3).cuda()
    r1[:, 1] = -r0[:, 2]
    r1[:, 2] = r0[:, 1]
    r1[:, 5] = -r0[:, 0]
    r1 = r1.view(-1, 3, 3)
...
def expmap2xyz(expmap):
    ...
    """
    parent, offset, rotInd, expmapInd = forward_kinematics._some_variables()
    xyz = forward_kinematics.fkl(expmap, parent, offset, rotInd, expmapInd)
    return xyz
