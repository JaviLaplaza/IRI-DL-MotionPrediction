...
def fkl(angles, parent, offset, rotInd, expmapInd):
    ...
    for i in np.arange(njoints):
        ...
        if parent[i] == -1:  # Root node
            x = angles[rotInd[i][2] - 4]
            y = angles[rotInd[i][0] - 4]
            z = angles[rotInd[i][1] - 4]
            xyz = np.array([x, y, z]) * 1000
            print(xyz)

            xyzStruct[i]['rotation'] = thisRotation
            # xyzStruct[i]['xyz'] = np.reshape(offset[i, :], (1, 3)) + thisPosition
            xyzStruct[i]['xyz'] = xyz + thisPosition
            # print(thisPosition)
            # print(offset[i, :], (1, 3))
...
def fkl_torch(angles, parent, offset, rotInd, expmapInd):
    ...
    angles = angles[:, :].contiguous().view(-1, 3)
