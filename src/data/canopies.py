...
class CANOPIESDataset(DatasetBase):
    def __init__(self, opt, is_for, subset, transform, dataset_type):
        super(CANOPIESDataset, self).__init__(opt, is_for, subset, transform, dataset_type)
        ...
        for subj in self.subs:
            ...
            for file in os.listdir(subj_path):
                if file == "collab.csv" or file == "adv.csv" or file == "gesture.csv":
                    file_path = os.path.join(subj_path, file)
                    try:
                        if file_path.endswith('.csv'):
                            print(f"Reading subject {subj}, action {file_path}")
...
if __name__ == "__main__":
    ...
    dataset = CANOPIESDataset(opt, is_for='train', subset=0, transform=transforms, dataset_type=0)
